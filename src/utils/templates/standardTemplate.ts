...
export function generateStandardPDReportHTML(data: PDReportPrintData): string {
  ...
  // Income Assessment Calculations
...
<body>
  ...
  <table class="report-table">
    ...
    ${data.shopAreaSqFt ? `
    <tr>
      <td class="bold">Shop Area (Sq. Ft.)</td>
      <td>${data.shopAreaSqFt}</td>
    </tr>
    ` : ''}
    ${data.inventoryValue ? `
    <tr>
      <td class="bold">Inventory / Stock Value</td>
      <td>₹${data.inventoryValue}</td>
    </tr>
    ` : ''}
    ${data.businessRemark ? `
    <tr>
      <td class="bold">Business Remarks</td>
